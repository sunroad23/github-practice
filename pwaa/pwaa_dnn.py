...
class DNN:
    ...
    def DNN(self):
        ...
        return np.vstack((raw[0:len(major_hp1)], diff_arr[0:len(major_hp1)], major_hp1, major_lp1, major_s1, recog_score)).T
    ...
    def Re_m(self, rs):
        ...
        hr = np.round(np.mean(hr_arr), 2)
        hrv = np.round(np.std(hrv_arr)*1000, 2)
        ...
        데이터 출력
        '''
        return np.array([n00, n80, hr, hrv])
